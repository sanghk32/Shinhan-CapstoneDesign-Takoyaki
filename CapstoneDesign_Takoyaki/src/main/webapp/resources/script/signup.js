...
function registerCheckFunction() {
	...
	$.ajax({
		type: 'POST',
		url: '../user/UserRegisterCheckServlet',
		data: { id: id },
		success: function(rs) {
			if (rs == 1) {
				$('#info__id .error-msg').html('사용 가능한 아이디입니다.').css('color', 'green');
				account.id = idInputEl.value
			} else {
				$('#info__id .error-msg').html('사용할 수 없는 아이디입니다.').css('color', 'red');
				account.id = null;
			}
			console.log(account)
		}
...
/***임의의 랜덤 수를 생성해서 70% 확률로 중복이 아니고 30% 확률로 중복된 아이디로 출력해서 ID 중복체크 테스트하는 기능***/
